...
* 최신 AEM 서비스 팩 또는 최신 업데이트를 통해 실행 중인 AEM 6.5, AEM 6.5 LTS 또는 AEM Forms as a Cloud Service 작성자 인스턴스.
...
1. [AEM 6.5 또는 AEM 6.5 LTS를 다운로드하여 설치하거나 AEM Forms as a Cloud Service을 온보딩합니다](#aemquickstart)
1. (AEM 6.5 및 AEM 6.5 LTS만 해당) [최신 AEM 서비스 팩 다운로드 및 설치](#servicepack)
1. (AEM 6.5 및 AEM 6.5 LTS만 해당) [최신 AEM Forms 추가 기능 패키지 다운로드 및 설치](#downloadaemformsaddon)
1. (선택 사항) [최신 커넥터 패키지 다운로드 및 설치](#installConnectorPackage)
1. [사용자 지정 테마 및 템플릿(AEM 6.5 / 6.5 LTS)을 만들거나 기본값(Cloud Service)을 사용합니다.](#referencepackage)

### AEM 6.5 또는 AEM 6.5 LTS 또는 온보드 AEM Forms as a Cloud Service을 다운로드하여 설치합니다. {#aemquickstart}


AFCS(자동 양식 전환 서비스)는 AEM 작성자 인스턴스에서 실행됩니다. AEM 작성자 인스턴스를 설정하려면 AEM 6.5, AEM 6.5 LTS 또는 AEM Forms as a Cloud Service이 필요합니다.
...
#### (AEM 6.5 및 AEM 6.5 LTS의 경우) 공개 인증서 받기 {#obtainpubliccertificates}
...
1. **[!UICONTROL Create Automated Forms Conversion Configuration]** 페이지의 **[!UICONTROL Advanced]** 탭에서 다음 필드에 대한 값을 지정하십시오.
   ...
   <tr>
   <td>Analytics 활성화</td>
   <td>(AEM 6.5 및 AEM 6.5 LTS의 경우) 변환된 모든 양식에서 Adobe Analytics을 활성화하려면 옵션을 선택합니다. 옵션을 사용하기 전에 AEM Forms 인스턴스에 대해 Adobe Analytics이 활성화되어 있는지 확인하십시오.</td>
   </tr>
   </tbody>
   </table>

   * 소스가 확장자가 .XDP인 XFA 기반 양식인 경우 출력 DOR은 XFA 레이아웃을 유지하며, 그렇지 않은 경우 전환 서비스는 기본 제공 템플릿을 사용하여 다른 XFA 기반 양식에 대한 DOR을 생성합니다.
   * XFA 양식을 제출하면 양식의 제출 데이터가 XML 요소 또는 속성으로 저장됩니다. 예를 들어, `<Amount currency="USD"> 10.00 </Amount>`과 같이 입력합니다. 통화는 속성으로 저장되고 통화 금액, 10.00은 요소로 저장됩니다. 적응형 양식의 데이터 제출에는 속성이 없으며 요소만 있습니다. 따라서 XFA 기반 양식이 적응형 양식으로 변환되면 적응형 양식 제출 데이터는 이러한 각 속성에 대한 요소를 포함합니다. 예:
